...
def load_settings(require_routing: bool = True) -> Settings:
    ...
    if require_routing and source_delivery_enabled:
        if not source_chats:
            raise ValueError("Environment variable SOURCE_CHATS is required when forwarding or email forwarding is enabled")
    if require_routing and forwarding_enabled:
        if target_chat_ref is None:
            raise ValueError("Environment variable TARGET_CHAT is required when forwarding is enabled")
